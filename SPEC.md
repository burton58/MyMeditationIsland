...
- **"Dein heutiger Fokus"** (`#focusSection`) — **eine** Karte statt eines Nebeneinanders von Rückblick und Vorschlag (so wie bei Calm, Headspace & Co., nie beides gleichzeitig), und **ohne** etwas zu wiederholen, das schon in "Dein Zustand" direkt darunter steht. **Seit 5. Aug. 2026 nur noch ein Zustand, sichtbar oder ganz weg (Christine: "dein heutiger Zustand erscheint nur, wenn man auch eine Meditation heute gehört hat" — gemeint war dieser Block):**
  - **Heute schon etwas gehört** (`heutigeMeditationen()`, aus dem echten Verlauf) — der ganze Block ist sichtbar: die heute abgeschlossenen Übungen mit Uhrzeit, Dauer und Bereich, statt eines Vorschlags. **▶-Knopf an jeder Zeile** (Christines Regel "Ich möchte immer Knöpfe", 5. Aug. 2026 — gilt seither überall, wo eine Zeile eine einzelne Meditation zeigt und antippbar ist, auch der Verlauf auf "Mein Weg", `renderHistoryList()`, §3.3 Punkt 7). Höchstens drei Zeilen, darunter eine Zusammenfassung bei mehr.
  - **Sonst** (noch keine Empfehlung gebaut, oder eine Empfehlung da, aber heute noch nichts gehört): der **ganze Block bleibt verborgen** (`focusWrap.hidden = true`) — kein Hinweistext, keine Empfehlungskarte mehr an dieser Stelle. Beides ist über "Dein Zustand" direkt darunter (Knopf "Zum Kompass →") ohnehin erreichbar.
  - **BEWUSST NICHT GELÖSCHT:** Die frühere Logik für die beiden anderen Zustände (Hinweistext "Sobald du deinen Kompass gestellt hast …" bzw. die startbare Top-Empfehlung mit "Weitere Empfehlungen ansehen →") steht in `renderHome()` weiterhin als auskommentierter Block ("Alten Code im Hintergrund behalten, man weiss ja nie"). Zum Zurückholen: Kommentar entfernen und `focusWrap.hidden` in allen drei Fällen auf `false` setzen.
