...
| 4. Aug. 2026 | **In der Bibliothek gibt es pro Übung nur noch ein Mantra, kein Mudra.** Christine: "Bei Meditationen nur Mantra optional anbieten und kein Mudra." | §3.3a |
| 4. Aug. 2026 | **Die Karte "Brauchst du noch etwas?" auf der Abschluss-Seite entfällt ganz** — erst das Mudra ("Entferne Mudra bei den Meditationen"), dann auch das Mantra ("Mantra auch weg bei den Meditationen am Ende, aber belasse [es in der Bibliothek]"). Der Abschluss klingt jetzt ohne weiteres Angebot aus. Ein Mantra gibt es weiterhin — aber dort, wo man eine Übung auswählt: **pro Übung in der Bibliothek**. | §3.5, §3.3a |
...
- **Mudra & Mantra pro Übung** (`mudraMantraFuerUebung()`, `toggleMudraMantraInfo()`, 4. Aug. 2026 — **ersetzt die Begleiter-Karte**, siehe §6a "Deaktiviert, nicht gelöscht"): Ein eigener, dezenter Knopf 🤲 je Zeile (`data-info`, gleiche Zurückhaltung wie der Stern — Play soll der klar stärkste Knopf der Zeile bleiben) klappt darunter eine Karte mit dem zur Übung passenden Mudra **und** Mantra auf, ohne die Übung zu starten (eigener Klick-Zweig vor `data-play`, mit `stopPropagation()`). **Fest zugeordnet, nicht geraten:** Beide kommen aus den bestehenden `MUDRAS`/`MANTRAS`-Listen, gefiltert auf die Kompass-Richtung (`dir`) der Übung — dieselbe Zuordnung, mit der auch der Kompass selbst arbeitet (§3.5, `waehlePassend()`). `textStreuung(m.id)` verteilt die Wahl innerhalb des Richtungs-Pools, damit nicht alle Übungen derselben Richtung dasselbe Paar zeigen; beim Mantra zusätzlich ein Versatz von 3, damit Mudra und Mantra nicht regelmässig auf denselben Pool-Platz treffen. Es ist immer nur eine Karte gleichzeitig offen (eine neu geöffnete schliesst die vorherige), sonst wächst die Liste bei mehrfachem Antippen unbegrenzt nach unten. **Nachbesserung (4. Aug. 2026, Christine hat den Knopf zuerst nicht gefunden):** `opacity:.55` machte das Emoji nur ausgewaschen statt dezent, kaum zu erkennen. Jetzt volle Deckkraft und etwas grösser (`1.15rem`); `color` bleibt zusätzlich gesetzt, wirkt bei den meisten Emoji-Schriften aber ohnehin nicht (Falle beim Ändern: Emoji ignorieren CSS-`color` auf den meisten Plattformen — die Sichtbarkeit steuert hier praktisch nur `opacity`/Grösse). **Icon getauscht (gleicher Tag):** 🤲 stand schon für den reinen Mudra-Knopf auf der Abschluss-Seite; dieser Knopf hier öffnet aber Mudra **und** Mantra zusammen. Jetzt 🪷 (Lotusblüte) — bereits an anderer Stelle in der App im Einsatz (Profilbild-Auswahl), passt thematisch und ist von 🤲 klar unterschieden. Eintrag dazu in `Icon-Prompts.md` (Gruppe 7) ergänzt, damit er beim künftigen Ersetzen aller Emoji durch eigene Symbole nicht vergessen geht. **Nachbesserung (4. Aug. 2026, Christine: "was bringt dieses Mudra? Dies soll auch stehen"):** Die Karte zeigte beim Mudra bisher nur `how` (wie man es ausführt), nicht `why` (wofür es gut ist) — beim Mantra stand `why` schon immer dabei. Jetzt zeigt die Mudra-Zeile beide Sätze, dazu — falls vorhanden — Dauer/Chakra/Mantra-Silbe in Gold als eigene Meta-Zeile, genau wie im bestehenden Mudra-Kärtchen auf der Abschluss-Seite (`zeigeMudra()`), damit beide Stellen dieselben Angaben in gleicher Form zeigen. **Kurswechsel, gleicher Tag: nur noch Mantra, kein Mudra mehr** (Christine: "Bei Meditationen nur Mantra optional anbieten und kein Mudra"). `mudraMantraFuerUebung()` ist damit entfallen, ersetzt durch das einfachere `mantraFuerUebung(m)` (nur noch die `MANTRAS`-Liste, kein zweiter Pool, kein Versatz mehr nötig). Die Karte zeigt jetzt nur noch eine Zeile: „✨ „Mantra“" + `why`. Das Icon wechselte dabei zurück zu **✨** — dasselbe Symbol wie beim Mantra-Kärtchen auf der Abschluss-Seite, weil beide jetzt genau dasselbe tun (nur ein Mantra zeigen); 🪷 war nur für die kurze Zeit nötig, in der der Knopf Mudra und Mantra gemeinsam öffnete. Das Mudra-Kärtchen auf der Abschluss-Seite (`zeigeMudra()`) blieb zunächst bestehen und ist am selben Tag ebenfalls entfallen (§3.5).
...
- **Entfallen (4. Aug. 2026): die Karte "Brauchst du noch etwas?"** — sie bot "Ein Mudra für mich" und "Ein Mantra für mich" an (deterministisch aus der Nadel-Position gewählt, `waehlePassend()`). Christine hat beides nacheinander am selben Tag gestrichen: erst das Mudra, dann auch das Mantra. Begründung durchgehend dieselbe wie schon bei "Noch eine Meditation", das im Aug. 2026 aus demselben Grund verschwand: **direkt nach einer Meditation gleich das nächste Angebot danebenzustellen passt nicht zum Ausklang.** Die Seite endet jetzt mit Rückblick, Vergleichssatz und den zwei Wegen weiter ("Kompass neu setzen" / "Fertig"). Mit entfernt: `#needMudraBtn`, `#needMantraBtn`, `#mudraBox`, `#mantraBox`, `zeigeMudra()`, `zeigeMantra()`, `waehlePassend()` und die Regeln `.need-more*` / `.mudra-card` / `.mantra-card`. **Nicht entfernt:** die Listen `MUDRAS` und `MANTRAS` — `MANTRAS` trägt das Mantra pro Übung in der Bibliothek (§3.3a, die **einzige** verbliebene Mantra-Stelle der App), `MUDRAS` gehört zum deaktivierten Begleiter (§6a). `.mini-btn` bleibt ebenfalls: Pause/Vorspulen/Fertig im Spieler nutzen die Klasse.
