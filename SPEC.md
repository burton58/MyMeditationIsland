...
7. **"Deine Favoriten"** (`#favoritenListe`, vom früheren Profil hierher gezogen, 4. Aug. 2026) — vorher eine Menüzeile im Profil, die auf die Bibliothek mit ⭐-Filter sprang; jetzt die **vollständige Liste direkt hier**, mit derselben Zeilenart wie in der Bibliothek (`.lib-row`: Symbol, Name, Dauer/Bereich, Stern zum Abwählen, Play zum Starten). `renderFavoritenSektion()` wird sowohl von `renderMyMed()` als auch vom globalen `data-fav`-Klick-Zuhörer aufgerufen, damit Bibliothek und "Mein Weg" nie auseinanderlaufen. Ohne Favoriten steht eine kurze Einladung ("Markiere eine Übung mit dem Stern, dann steht sie hier.") statt einer leeren Karte.
8. **"Deine letzten Meditationen"** — seit Aug. 2026 zunächst nur die **drei** jüngsten Einträge aus `loadHistory()` mit "Heute"/"Gestern"/Wochentag, Dauer und Bereich (`renderHistoryList()`, vorher fünf ohne Ausklappen). Darunter **"Alle N anzeigen"** / **"Weniger anzeigen"** (`#historyMehrBtn`, Zustand in `historyAlle`); der Knopf bleibt ganz weg, solange es höchstens drei Einträge gibt.
9. **Entfallen (Aug. 2026):** "Alle Meditationen entdecken" (die vier Kategorie-Karten und "Ganze Bibliothek ansehen →") und der **Begleiter-Chat**. Beides doppelte genau das, was der Reiter "Meditationen" daneben macht — Christine hat die Dopplung selbst bemerkt. "Mein Weg" ist die Auswertungsseite und bleibt jetzt dabei; Stöbern und Nachfragen passieren in der Bibliothek. Mit entfernt: `initKategorien()`, der `#fdAllBtn`-Listener, die `setupChat("chatBox", …)`-Instanz und die toten `.fd-cat*`/`.companion`-Regeln (`.companion-foto` bleibt — der runde Bild-Platz sitzt jetzt in der `.ki-card` der Bibliothek).
...
- **Favoriten** liegen wie der Verlauf nur auf dem Gerät (`localStorage`, Schlüssel `myisland.favoriten.v1`); sichtbar über den Chip ⭐ in der Bibliothek und als eigener Abschnitt "Deine Favoriten" auf "Mein Weg" (§3.3, seit 4. Aug. 2026).
