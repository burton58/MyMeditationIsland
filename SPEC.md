...
- Seitenkopf "Profil · Dein Bereich", Karte "Dein Inselbewohner" — **seit Aug. 2026 wirklich antippbar** (`#profilInselbewohnerBtn`, ein `<button>` statt eines blossen `<div>`): scrollt weich zu "Deine Inselreise" (oder ersatzweise "Deine Inselwoche", falls die Reise-Karte noch leer ist) gleich weiter unten auf derselben Seite. Vorher versprach der Pfeil "›" Antippbarkeit, ohne dass ein Klick etwas ausgelöst hätte — ein echter, unentdeckter Lücke, bis Christine beim Durchklicken selbst darauf stiess.
- **Status-Karte** (`renderStatusCard()`) zum Zugang, danach zwei Karten aus dem echten Verlauf (blenden sich aus, solange nichts da ist):
  - **"Deine Inselwoche"** (`renderWeek()`) — sieben Kreise von Montag bis Sonntag, gefüllt an den Tagen mit einer Sitzung, der heutige Tag zusätzlich umrandet. Darunter "X Tage am Stück" (`computeStreak()`), "Ziel: 7 Tage" und ein Balken. Hinter **"Details"** stehen wie früher die Zeilen mit Vorher/Nachher je Tag. Erscheint erst nach der ersten echten (≥20 Sekunden langen) Meditation — bis dahin bewusst unsichtbar statt einer leeren Woche.
...
- **Einstellungen** als Liste: "Abo verwalten" (→ Abo-Seite), "Statistiken" (→ Mein Weg), "Favoriten" (→ Bibliothek, Filter ⭐), "Meine Insel gestalten" (→ §4), "Über die App" (blendet zwei Sätze ein), "Verlauf löschen". **"Statistiken" und "Favoriten" haben seit Aug. 2026 bewusst keinen Pfeil "›" mehr** — beide springen nur auf einen anderen Reiter (kein echtes Unterseiten-Drilldown mit Rückweg wie bei Impressum/Über mich), der Pfeil hätte das Gegenteil suggeriert. **Die frühere Zeile "Dein Begleiter" ist entfernt** — der Begleiter war schon über die Bibliothek und "Mein Weg" gut erreichbar; eine dritte Stelle im Einstellungs-Menü war redundant (Christines Entscheid nach Rückfrage).
...
6. **Barrierefreiheit**: Kompass jetzt per Pfeiltasten bedienbar (Fokus auf der Scheibe, `tabindex="0"`, `role="group"`), zusätzlich zum bestehenden Pointer-Drag — beide Wege laufen über dieselbe `setFromPoint()`, also identische Rundung/Begrenzung/Spiegelung. `aria-label` beschreibt den aktuellen Zustand in Worten und aktualisiert sich bei jeder Änderung (§3.2). Sichtbarer Fokus-Ring in Gold, nur bei echter Tastatur-Bedienung (`:focus-visible`). **Fehler behoben (Aug. 2026, Christine gemeldet):** `tabindex="0"` bedeutet, dass die Scheibe bei *jeder* Interaktion fokussiert wird, auch beim gewöhnlichen Ziehen mit Finger/Maus — ohne ein explizites `outline:none` auf dem Grundzustand zeigte der Browser dabei zusätzlich seinen eigenen blauen System-Fokusrahmen um den Kompass, unabhängig vom gestalteten Gold-Ring. Jetzt unterdrückt, der goldene Ring bleibt der einzige Fokus-Hinweis. Offen bleibt eine vollständige Sprachausgabe-Führung durch die restliche App.
