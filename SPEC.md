...
- **"Ein Mantra für diesen Moment"** — ein **Mantra** passend zum Zustand *nach* der Meditation (`zeigeMantra()`, `waehlePassend()` wählt deterministisch aus der Nadel-Position, nicht zufällig; `renderRecap()` setzt `#mantraBox` beim Bestätigen zurück, weil ein dort stehendes Mantra sonst zum alten Zustand gehören würde). **Dies ist die einzige Mantra-Stelle der App.** — **Entfallen (4. Aug. 2026): der Mudra-Knopf, der danebenstand** (`#needMudraBtn`, `#mudraBox`, `zeigeMudra()`, die `.mudra-card`-Regeln). Mudras kommen in der Oberfläche nirgends mehr vor; die Liste `MUDRAS` bleibt nur für den deaktivierten Begleiter in der Datei (§6a). — **Entstehungsgeschichte (alles 4. Aug. 2026, drei Schritte an einem Tag):** zuerst fiel das Mudra weg; dann wurde versuchsweise auch das Mantra hier entfernt und stattdessen pro Übung in die Bibliothek gelegt; Christine hat das zurückgedreht — das Mantra passt zum Zustand nach der Meditation, nicht zu einer Übung, die man noch gar nicht gehört hat. Ebenfalls schon früher entfallen: **"Noch eine Meditation"** (Aug. 2026) — direkt nach einer Meditation gleich die nächste anzubieten passte nicht zum Ausklang; wer weitermachen will, geht über "Kompass neu setzen", dann stimmt auch der Zustand wieder. **Umformuliert (4. Aug. 2026, Christine: "eine Frage, brauchst du noch etwas, und es gibt keine Auswahl, sondern nur ein Mantra — macht keinen Sinn"):** Die Karte hiess "Brauchst du noch etwas?" mit einem Knopf "Ein Mantra für mich" — eine Frage nach einer Auswahl, die es seit dem Wegfall des Mudra-Knopfes gar nicht mehr gab. Jetzt sagt die Überschrift, **was** es gibt ("Ein Mantra für diesen Moment"), eine Zeile darunter erklärt es kurz, und der Knopf sagt, **was passiert** ("✨ Mantra anzeigen", volle Breite statt kleiner Chip). **Dabei aufgefallen und mitbehoben:** Die Karte war schon **vor** dem Bestätigen bedienbar — das Mantra wird aus `compassAfter` gewählt, also aus einer Nadel, die zu dem Zeitpunkt nur in ihrer Ruhestellung steht. Ein "passend dazu, wie du dich jetzt fühlst" wäre dann eine Behauptung über einen Zustand, den niemand angegeben hat (**Grundsatz G1**). Die Karte (`#mantraKarte`) ist darum jetzt bis zum Bestätigen ausgeblendet — dieselbe Regel wie bei Rückblick, Vergleichssatz und "Zustand nochmals ändern", alle vier schaltet `setOutroBestaetigt()` gemeinsam.
